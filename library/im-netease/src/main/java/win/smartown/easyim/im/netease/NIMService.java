...
public class NIMService extends IMService {
    ...
    @Override
    public void refreshMessages(String account) {
        IMMessage anchor = MessageBuilder.createEmptyMessage(account, SessionTypeEnum.P2P, System.currentTimeMillis());
        NIMSDK.getMsgService().queryMessageListEx(anchor, QueryDirectionEnum.QUERY_OLD, 1000, true).setCallback(messageCallback);
    }

    @Override
    public void sendMessage(String account, String text) {
        final IMMessage message = MessageBuilder.createTextMessage(account, SessionTypeEnum.P2P, text);
