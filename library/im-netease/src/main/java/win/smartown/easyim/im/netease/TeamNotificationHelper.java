...
public class TeamNotificationHelper {
    ...
    private static String buildInviteMemberNotification(MemberChangeAttachment a, String fromAccount) {
        ...
        Team team = NIMSDK.getTeamService().queryTeamBlock(teamId.get());
        ...
        for (Map.Entry<TeamFieldEnum, Object> field : a.getUpdatedFields().entrySet()) {
            if (field.getKey() == TeamFieldEnum.Name) {
                ...
                if (type == VerifyTypeEnum.Free) {
                    sb.append(authen + IM.getInstance().getContext().getString(R.string.team_allow_anyone_join));
                } else if (type == VerifyTypeEnum.Apply) {
                    sb.append(authen + IM.getInstance().getContext().getString(R.string.team_need_authentication));
                } else {
                    sb.append(authen + IM.getInstance().getContext().getString(R.string.team_not_allow_anyone_join));
                }
